...
using Traccar.Model;
using Traccar.Protocols.Starcom;
using Xunit;

namespace Traccar.Protocols.Tests;

public sealed class StarcomProtocolDecoderTest : ProtocolTestBase
{
    private StarcomProtocolDecoder CreateDecoder()
        => new(CreateConnectionManager(), NullLogger<StarcomProtocolDecoder>.Instance);

    [Fact]
    public void TestDecodeFullPosition()
    {
        var decoder = CreateDecoder();

        var pos = VerifyPosition(decoder, Buffer(
        ...
        Assert.Equal(9.06824, pos.Latitude, 0.00001);
        Assert.Equal(40.86503, pos.Longitude, 0.00001);
        Assert.Equal(1809, pos.Altitude);
        Assert.Equal(130, pos.Course);
        Assert.True(pos.HasAttribute(Position.KeyIgnition));
        Assert.Equal("True", pos.Attributes[Position.KeyIgnition]?.ToString());
        Assert.Equal("14.11", pos.Attributes[Position.KeyPower]?.ToString());
    }

    [Fact]
    public void TestDecodeMinimalPosition()
    {
        var decoder = CreateDecoder();
    ...
    [Fact]
    public void TestDecodeInvalidDateIgnored()
    {
        var decoder = CreateDecoder();

        // datetime_actual=1899/12/30 is a sentinel for no valid time — must not throw
    ...
    [Fact]
    public void TestDecodeOdometer()
    {
        var decoder = CreateDecoder();
        ...
        Assert.Equal("1300000", pos.Attributes[Position.KeyOdometer]?.ToString());
    }
}
