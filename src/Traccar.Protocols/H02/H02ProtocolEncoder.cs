...
public sealed class H02ProtocolEncoder(IDbContextFactory<TraccarDbContext> dbContextFactory, ILogger<H02ProtocolEncoder> logger)
    : StringProtocolEncoder("h02", dbContextFactory, logger)
{
    ...
    internal object? EncodeCommand(Command command, DateTime time)
    {
        ...
        return command.Type switch
        {
            ...
            Command.TypePositionPeriodic => FormatCommand(
                time, uniqueId, "S71", "22", command.Attributes[Command.KeyFrequency].ToString()!),
            _ => null,
        };
    }
